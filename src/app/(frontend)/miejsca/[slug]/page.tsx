...
import { MapPin, Star, Ticket, BadgeCheck, Images } from 'lucide-react';
...
import { PlaceStoryBlocks } from '@/components/common/PlaceStoryBlocks';
...
export default async function PlacePage({ params }: { params: Promise<{ slug: string }> }) {
    ...
    const isPremium = place.plan === 'premium' &&
        (!place.premiumExpiresAt || new Date(place.premiumExpiresAt) > new Date());
    ...
    const firstGalleryImageUrl = galleryItems.length > 0 && typeof galleryItems[0]?.image === 'object'
        ? (galleryItems[0].image as Media)?.url ?? null
        : null;

    const showHero = false; // hero disabled — keeping standard header for all plans

    const storyBlocks = place.storyBlocks ?? [];
    ...
    return (
        <main className="bg-gray-50 pb-24 min-h-screen relative">
            ...
            {showHero ? (
                    ...
                    <div className="absolute top-0 left-0 right-0 z-20 px-6 pt-8">
                        <div className="max-w-7xl mx-auto flex justify-between items-start">
                            <Breadcrumbs
                                ...
                                className="bg-white/20 backdrop-blur-md text-white px-4 py-2 rounded-full shadow-sm w-fit [&_a]:text-white/80 [&_a]:hover:text-white [&_span]:text-white/60"
                            />
                            <div className="flex gap-3">
                                <HeartButton id={place.id} type="place" className="bg-white/20 backdrop-blur-md border border-white/30 text-white hover:bg-white/30 p-4 rounded-full transition-all" />
                                <ShareButton title={place.name} className="bg-white/20 backdrop-blur-md border border-white/30 text-white p-4 rounded-full hover:bg-white/30 transition-all" />
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="max-w-7xl mx-auto">
                            ...
                            <div className="flex items-center gap-3 mb-5">
                                ...
                                {typeof place.category !== 'number' && place.category && (
                                    <span className="px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest text-white bg-white/20 backdrop-blur-md border border-white/30">
                            ...
                            <div className="flex flex-col md:flex-row md:items-end gap-5">
                                {/* Logo */}
                                {imageUrl && (
                                    <div className="w-20 h-20 md:w-24 md:h-24 shrink-0 rounded-2xl overflow-hidden border-2 border-white/30 shadow-xl bg-white/10 backdrop-blur-md p-1.5">
                                        <div className="relative w-full h-full rounded-xl overflow-hidden">
                                            <Image src={imageUrl} alt={place.name} fill className="object-contain" sizes="96px" />
                                        </div>
                                    </div>
                                )}

                                <div className="flex-1">
                                    <h1 className="text-4xl md:text-6xl lg:text-7xl font-black text-white mb-3 leading-tight drop-shadow-lg">
                                        {place.name}
                                    </h1>
                                    <div className="flex flex-wrap items-center gap-6 text-white/80 font-medium">
                                        <div className="flex items-center gap-1.5">
                                            <Star className={place.rating && place.rating > 0 ? "text-amber-400 fill-amber-400" : "text-white/40"} size={18} />
                                            {place.rating && place.rating > 0 ? (
                                                ...
                                                <span className="text-sm text-white/50">Brak ocen</span>
                                            )}
                                        </div>
                                ...
                                {galleryItems.length > 1 && (
                                    <div className="flex items-center gap-2 bg-white/20 backdrop-blur-md border border-white/30 text-white px-4 py-2 rounded-xl text-sm font-bold shrink-0">
                ...
                /* ── STANDARD HEADER (free / premium without gallery) ─────── */
                <>
            ...
            {/* ── GALLERY GRID (premium, below hero or below standard header) */}
            ...
            <div className="max-w-7xl mx-auto px-6 grid md:grid-cols-3 gap-8 mt-8">
                ...
                <div className="md:col-span-2 space-y-8">
                    ...
                    {/* Story Blocks */}
                    {isPremium && storyBlocks.length > 0 && (
                        <PlaceStoryBlocks blocks={storyBlocks as Parameters<typeof PlaceStoryBlocks>[0]['blocks']} />
                    )}
